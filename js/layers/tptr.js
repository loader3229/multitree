...
addLayer("tptr_p", {
	...
	if (hasUpgrade("tptr_b", 14) && player.tptr_i.buyables[12].gte(1))ret=ret.mul(upgradeEffect("tptr_b", 11))
			...
			12: {
				...
				effect() {
					...
					if (hasUpgrade("tptr_g", 34) && player.tptr_i.buyables[12].gte(2)) eff = eff.pow(1.4333333)
					...
					if (hasUpgrade("tptr_b", 34) && player.tptr_i.buyables[12].gte(1)) eff = eff.pow(upgradeEffect("tptr_b", 34));
	...
	effect() {
		...
		if(hasUpgrade("tm",64))ret = ret.pow(10);
		return ret;
	},
...
addLayer("tptr_b", {
	...
	getResetGain() {
		...
		if(ret.gte(1000))ret=ret.div(sc1).sqrt().mul(sc1).div((hasUpgrade("tptr_t", 34) && player.tptr_i.buyables[12].gte(4))?1400:1225).pow(0.1).mul((hasUpgrade("tptr_t", 34) && player.tptr_i.buyables[12].gte(4))?1400:1225).floor().max(1000);
	...
	getNextAt(canMax) {
		...
		let sc1=layers[this.layer].getSc1();
		if(amt.gte(1000))amt=amt.div((hasUpgrade("tptr_t", 34) && player.tptr_i.buyables[12].gte(4))?1400:1225).pow(10).mul((hasUpgrade("tptr_t", 34) && player.tptr_i.buyables[12].gte(4))?1400:1225).div(sc1).pow(2).mul(sc1).max(1000).ceil();
    ...
    layerShown(){return player.tm.currentTree==7 && hasUpgrade("tm",17)},
		...
		effectBase() {
			...
			if (hasUpgrade("tptr_b", 24) && player.tptr_i.buyables[12].gte(1)) base = base.times(upgradeEffect("tptr_b", 24));
		...
		upgrades: {
			...
			11: {
				...
				effect() { 
					...
					if (hasUpgrade("tptr_b", 14) && player.tptr_i.buyables[12].gte(1)) ret = ret.pow(upgradeEffect("tptr_b", 14));
					return ret;
				},
			...
			14: {
				title: "Meta-Combo",
				description: "The first 3 Booster Upgrades are stronger based on your Super Boosters, and <b>BP Combo</b> directly multiplies Rewritten Point gain.",
				cost() { return new Decimal(2275) },
				unlocked() { return player.tptr_i.buyables[12].gte(1)&&hasUpgrade("tptr_b", 13) },
				effect() { return player.tptr_sb.points.plus(1) },
			...
			24: {
				...
				cost() { return new Decimal(2250) },
				unlocked() { return player.tptr_i.buyables[12].gte(1)&&hasUpgrade("tptr_b", 23) },
				effect() { return player.tptr_b.points.plus(1).pow(500) },
			...
			34: {
				...
				cost() { return new Decimal(2280) },
				unlocked() { return player.tptr_i.buyables[12].gte(1)&&hasUpgrade("tptr_b", 33) },
				effect() { return player.tptr_i.points.plus(1).root(4) },
				effectDisplay() { return "^"+format(tmp[this.layer].upgrades[this.id].effect) },
				formula: "(x+1)^0.25",
			},
		},
		...
		upgrades: {
			...
			22: {
				...
				effect() { 
					...
					if (hasUpgrade("tptr_g", 32) && player.tptr_i.buyables[12].gte(2)) eff = eff.pow(upgradeEffect("tptr_g", 32));
					return eff;
				},
				unlocked() { return hasUpgrade("tm",22) },
			...
			31: {
				...
				cost() { return new Decimal(2255) },
				unlocked() { return player.tptr_i.buyables[12].gte(2); },
				effect() { return player.tptr_g.power.plus(1).log10().plus(1).pow(2) },
				effectDisplay() { return format(tmp[this.layer].upgrades[this.id].effect)+"x" },
			},
			32: {
				title: "Primal Instincts",
				description: "The <b>Quaternary Space Building</b> also affects <b>Discount Two</b> at a reduced rate.",
				cost() { return new Decimal(2235) },
				unlocked() { return player.tptr_i.buyables[12].gte(2); },
				effect() { return buyableEffect("tptr_s", 14).pow(0.3) },
				effectDisplay() { return "^"+format(tmp[this.layer].upgrades[this.id].effect) },
				formula: "eff^0.8",
				style: {"font-size": "9px"},
			},
			33: {
				...
				cost() { return new Decimal(2314) },
				unlocked() { return player.tptr_i.buyables[12].gte(2); },
				effect() { return Decimal.pow(1.15, player.tptr_g.points.sqrt()) },
				effectDisplay() { return format(tmp[this.layer].upgrades[this.id].effect)+"x" },
				formula: "1.15^sqrt(x)",
			},
			34: {
				...
				cost() { return new Decimal(2310) },
				unlocked() { return player.tptr_i.buyables[12].gte(2); },
			},
			35: {
				title: "Into The Future",
				description: "Nebula Energy, Honour, and Hyperspace Energy gains are boosted by Generator Power.",
				cost() { return new Decimal(2230) },
				unlocked() { return player.tptr_i.buyables[12].gte(2); },
				effect() { return player.tptr_g.power.plus(1).log10().plus(1).root(4) },
				effectDisplay() { return format(tmp[this.layer].upgrades[this.id].effect)+"x" },
			},
		},
...
addLayer("tptr_t", {
    ...
    layerShown(){return player.tm.currentTree==7 && hasUpgrade("tm",26)},
		...
		effLimBaseMult() {
			let mult = tmp.tptr_n.realDustEffs2?new Decimal(tmp.tptr_n.realDustEffs2.orangePurple||1):new Decimal(1);
			if (hasUpgrade("tptr_t", 33) && player.tptr_i.buyables[12].gte(4)) mult = mult.times(upgradeEffect("tptr_t", 33));
			return mult;
		},
		nonExtraTCPow() {
		...
		enEff2() {
			...
			let exp = 5/9
			if (hasUpgrade("tptr_t", 35) && player.tptr_i.buyables[12].gte(4)) exp = .565;
		...
		nextEnEff2() {
			...
			let next = Decimal.pow(10, ret.pow((hasUpgrade("tptr_t", 35) && player.tptr_i.buyables[12].gte(4))?1/.565 : 1.8));
			return next;
		},
		milestones: {
			0: {
		...
		upgrades: {
			...
			33: {
				...
				cost() { return new Decimal(1110) },
				unlocked() { return player.tptr_i.buyables[12].gte(4) },
				effect() { return player.tptr_t.energy.plus(1).log10().plus(1).pow(3.5) },
				effectDisplay() { return format(tmp.tptr_t.upgrades[33].effect)+"x" },
			},
			34: {
				title: "Scalings Galore",
				description: "Post-1k Booster & Generator cost scalings are weaker.",
				cost() { return new Decimal(1109) },
				unlocked() { return player.tptr_i.buyables[12].gte(4) },
			},
			35: {
				title: "Don't Kill Time",
				description: "Time Energy's second effect exponent is increased (0.556 -> 0.565)",
				cost() { return new Decimal(1114) },
				unlocked() { return player.tptr_i.buyables[12].gte(4) },
			},
			41: {
				title: "Subtemporal Power",
				description: "Raise the Subspace base to the power of 1.5, and gain 2,500x more Hyperspace Energy.",
				cost() { return new Decimal(1118) },
				unlocked() { return player.tptr_i.buyables[12].gte(4) },
			},
		},
		freeExtraTimeCapsules() {
...
addLayer("tptr_e", {
    ...
    layerShown(){return player.tm.currentTree==7 && hasUpgrade("tm",27)},
		...
		freeEnh() {
			...
			if (hasUpgrade("tptr_e", 32) && player.tptr_i.buyables[12].gte(3)) enh = enh.plus(upgradeEffect("tptr_e", 32));
			return enh;
		},
		...
		buyables: {
			...
			11: {
				title: "Enhancers",
				costScalingEnabled() {
					return !(hasUpgrade("tptr_e", 34) && player.tptr_i.buyables[12].gte(3));
				},
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
		...
		upgrades: {
			...
			31: {
				...
				cost() { return new Decimal("e135e5") },
				unlocked() { return player.tptr_i.buyables[12].gte(3) },
			},
			32: {
				title: "Supplementation",
				description: "Best Honour provides free Enhancers.",
				cost() { return new Decimal("e1473e4") },
				unlocked() { return player.tptr_i.buyables[12].gte(3) },
				effect() { return player.tptr_hn.best.plus(1).log10().pow(3).floor().min("1.25e8"); },
				effectDisplay() { return "+"+format(tmp[this.layer].upgrades[this.id].effect) },
			},
			33: {
				title: "Augmentation",
			...
			34: {
				...
				cost() { return new Decimal("e1345e4") },
				unlocked() { return player.tptr_i.buyables[12].gte(3) },
			},
			41: {
				title: "To the Next Level",
				description: "Enhance Points boost Hyperspace Energy gain.",
				cost() { return new Decimal("e1738e4") },
				unlocked() { return player.tptr_i.buyables[12].gte(3) },
				effect() { return player.tptr_e.points.plus(1).log10().plus(1).pow(.15) },
				effectDisplay() { return format(tmp[this.layer].upgrades[this.id].effect)+"x" },
			},
		},
		update(){
    ...
    layerShown(){return player.tm.currentTree==7 && hasUpgrade("tm",28)},
		...
		updateTemp()
			...
			16: {
				...
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
					let base = tmp.tptr_s.buildingBaseCosts[this.id];
					let cost = Decimal.pow(base, x.times(tmp.tptr_s.buildScalePower).pow(x.gte(10)?1.65:1.38)).times(base);
					return cost.div(tmp.tptr_s.divBuildCosts);
                },
				freeLevels() {
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let ret = x.plus(tmp.tptr_s.buyables[this.id].freeLevels).times(tmp.tptr_s.buildingPower).plus(1).sqrt();
					if (player.tptr_hs.unlocked) ret = ret.pow(buyableEffect("tptr_hs", 26));
					return ret.floor();
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    return ("Cost: " + formatWhole(data.cost) + " Generator Power")+"\n\
                    Level: " + formatWhole(player[this.layer].buyables[this.id])+(data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
					"+("Multiply Damned Soul gain by " + formatWhole(data.effect)+".")
                },
                unlocked() { return player[this.layer].unlocked&&player.tptr_i.buyables[11].gte(1) }, 
                canAfford() {
                    return player.tptr_g.power.gte(tmp[this.layer].buyables[this.id].cost) && layers.tptr_s.space().gt(0)},
                buy() { 
                    cost = tmp[this.layer].buyables[this.id].cost
                ...
                target() { return player.tptr_g.power.times(tmp.tptr_s.divBuildCosts).div(tmp.tptr_s.buildingBaseCosts[this.id]).max(1).log(tmp.tptr_s.buildingBaseCosts[this.id]).root(1.65).div(tmp.tptr_s.buildScalePower).plus(1).floor().min(player[this.layer].buyables[this.id].plus(layers.tptr_s.space())) }, 
                buyMax() {
					if (!this.canAfford() || !this.unlocked()) return;
					let target = this.target();
					player.tptr_s.spent = player.tptr_s.spent.plus(target.sub(player[this.layer].buyables[this.id]))
			...
			17: {
				...
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
					let base = tmp.tptr_s.buildingBaseCosts[this.id];
					let cost = Decimal.pow(base, x.times(tmp.tptr_s.buildScalePower).pow(x.gte(7)?1.95:1.41)).times(base);
					return cost.div(tmp.tptr_s.divBuildCosts);
                },
				freeLevels() {
					let levels = new Decimal(0);
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let ret = Decimal.pow("1e20000", x.plus(tmp.tptr_s.buyables[this.id].freeLevels).times(tmp.tptr_s.buildingPower).pow(1.2));
					if (player.tptr_hs.unlocked) ret = ret.pow(buyableEffect("tptr_hs", 27));
					return ret.floor();
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    return ("Cost: " + formatWhole(data.cost) + " Generator Power")+"\n\
                    Level: " + formatWhole(player[this.layer].buyables[this.id])+(data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
					"+("Divide the requirement of Phantom Souls by " + formatWhole(data.effect)+".")
                },
                unlocked() { return player[this.layer].unlocked&&player.tptr_i.buyables[11].gte(2) }, 
                canAfford() {
                    return player.tptr_g.power.gte(tmp[this.layer].buyables[this.id].cost) && layers.tptr_s.space().gt(0)},
                buy() { 
                    cost = tmp[this.layer].buyables[this.id].cost
                ...
                target() { return player.tptr_g.power.times(tmp.tptr_s.divBuildCosts).div(tmp.tptr_s.buildingBaseCosts[this.id]).max(1).log(tmp.tptr_s.buildingBaseCosts[this.id]).root(1.95).div(tmp.tptr_s.buildScalePower).plus(1).floor().min(player[this.layer].buyables[this.id].plus(layers.tptr_s.space())) }, 
                buyMax() {
					if (!this.canAfford() || !this.unlocked()) return;
					let target = this.target();
					player.tptr_s.spent = player.tptr_s.spent.plus(target.sub(player[this.layer].buyables[this.id]))
			...
			18: {
				...
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
					let base = tmp.tptr_s.buildingBaseCosts[this.id];
					let cost = Decimal.pow(base, x.times(tmp.tptr_s.buildScalePower).pow(2.25)).times(base);
					return cost.div(tmp.tptr_s.divBuildCosts);
                },
				freeLevels() {
					let levels = new Decimal(0);
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let ret = x.plus(tmp.tptr_s.buyables[this.id].freeLevels).times(tmp.tptr_s.buildingPower).div(1.5);
					if (player.tptr_hs.unlocked) ret = ret.times(buyableEffect("tptr_hs", 28));
					return ret;
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    return ("Cost: " + formatWhole(data.cost) + " Generator Power")+"\n\
                    Level: " + formatWhole(player[this.layer].buyables[this.id])+(data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
					"+("Get " + format(data.effect)+" more Free Quirk Layers.")
                },
                unlocked() { return player[this.layer].unlocked&&player.tptr_i.buyables[11].gte(3) }, 
                canAfford() {
                    return player.tptr_g.power.gte(tmp[this.layer].buyables[this.id].cost) && layers.tptr_s.space().gt(0)},
                buy() { 
                    cost = tmp[this.layer].buyables[this.id].cost
                ...
                target() { return player.tptr_g.power.times(tmp.tptr_s.divBuildCosts).div(tmp.tptr_s.buildingBaseCosts[this.id]).max(1).log(tmp.tptr_s.buildingBaseCosts[this.id]).root(2.25).div(tmp.tptr_s.buildScalePower).plus(1).floor().min(player[this.layer].buyables[this.id].plus(layers.tptr_s.space())) }, 
                buyMax() {
					if (!this.canAfford() || !this.unlocked()) return;
					let target = this.target();
					player.tptr_s.spent = player.tptr_s.spent.plus(target.sub(player[this.layer].buyables[this.id]))
			...
			19: {
				...
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
					let base = tmp.tptr_s.buildingBaseCosts[this.id];
					if(x.eq(0))return new Decimal(0);
					let cost = Decimal.pow(base, x.times(tmp.tptr_s.buildScalePower).pow(2.55)).times(base);
					return cost.div(tmp.tptr_s.divBuildCosts);
                },
				freeLevels() {
					let levels = new Decimal(0);
					if (hasUpgrade("tptr_s", 32) && player.tptr_i.buyables[12].gte(5)) levels = levels.plus(player.tptr_s.buyables[19+1]||0);
					return levels;
				},
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let ret = x.plus(tmp.tptr_s.buyables[this.id].freeLevels).times(tmp.tptr_s.buildingPower).div(1e3).plus(1);
					if (player.tptr_hs.unlocked) ret = ret.pow(buyableEffect("tptr_hs", 29));
					return ret;
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    return ("Cost: " + formatWhole(data.cost) + " Generator Power")+"\n\
                    Level: " + formatWhole(player[this.layer].buyables[this.id])+(data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
					"+("The Hyperspace Energy gain exponent is multiplied by " + format(data.effect)+".")
                },
                unlocked() { return player[this.layer].unlocked&&player.tptr_i.buyables[11].gte(4) }, 
                canAfford() {
                    return player.tptr_g.power.gte(tmp[this.layer].buyables[this.id].cost) && layers.tptr_s.space().gt(0)},
                buy() { 
                    cost = tmp[this.layer].buyables[this.id].cost
                ...
                target() { return player.tptr_g.power.times(tmp.tptr_s.divBuildCosts).div(tmp.tptr_s.buildingBaseCosts[this.id]).max(1).log(tmp.tptr_s.buildingBaseCosts[this.id]).root(2.55).div(tmp.tptr_s.buildScalePower).plus(1).floor().min(player[this.layer].buyables[this.id].plus(layers.tptr_s.space())) }, 
                buyMax() {
					if (!this.canAfford() || !this.unlocked()) return;
					let target = this.target();
					player.tptr_s.spent = player.tptr_s.spent.plus(target.sub(player[this.layer].buyables[this.id]))
			...
			20: {
				...
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
					let base = tmp.tptr_s.buildingBaseCosts[this.id];
					if(x.eq(0))return new Decimal(0);
					let cost = Decimal.pow(base, x.times(tmp.tptr_s.buildScalePower).pow(2.85)).times(base);
					return cost.div(tmp.tptr_s.divBuildCosts);
                },
				freeLevels() {
					let levels = new Decimal(0);
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let ret = x.plus(tmp.tptr_s.buyables[this.id].freeLevels).times(tmp.tptr_s.buildingPower).div(1e3);
					if (player.tptr_hs.unlocked) ret = ret.times(buyableEffect("tptr_hs", 30));
					return ret.add(1);
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    return ("Cost: " + formatWhole(data.cost) + " Generator Power")+"\n\
                    Level: " + formatWhole(player[this.layer].buyables[this.id])+(data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
					"+("Hyper Building Power is increased by " + format(data.effect.sub(1).times(100))+"%.")
                },
                unlocked() { return player[this.layer].unlocked&&player.tptr_i.buyables[11].gte(5) }, 
                canAfford() {
                    return player.tptr_g.power.gte(tmp[this.layer].buyables[this.id].cost) && layers.tptr_s.space().gt(0)},
                buy() { 
                    cost = tmp[this.layer].buyables[this.id].cost
                ...
                target() { return player.tptr_g.power.times(tmp.tptr_s.divBuildCosts).div(tmp.tptr_s.buildingBaseCosts[this.id]).max(1).log(tmp.tptr_s.buildingBaseCosts[this.id]).root(2.85).div(tmp.tptr_s.buildScalePower).plus(1).floor().min(player[this.layer].buyables[this.id].plus(layers.tptr_s.space())) }, 
                buyMax() {
					if (!this.canAfford() || !this.unlocked()) return;
					let target = this.target();
					player.tptr_s.spent = player.tptr_s.spent.plus(target.sub(player[this.layer].buyables[this.id]))
		...
		upgrades: {
			...
			35: {
				...
				cost() { return new Decimal(1151) },
				unlocked() { return player.tptr_i.buyables[12].gte(5) },
				effect() { return tmp.tptr_s.space.plus(1) },
				effectDisplay() { return format(tmp.tptr_s.upgrades[this.id].effect)+"x" },
			},
		},
});
...
addLayer("tptr_sb", {
        ...
        requires(){
			...
			if(player.tm.buyables[7].gte(36))ret = ret.sub(100);
		...
		effectBase() {
			...
			if (hasUpgrade("tptr_e", 31) && player.tptr_i.buyables[12].gte(3)) base = base.plus(buyableEffect("tptr_e", 11).second);
		...
		effect() {
			...
			return [Decimal.pow(this.effectBase(), player.tptr_sb.points).max(0),player.tptr_sb.points.add(1).pow(hasUpgrade("tptc_sb",22)?4:hasUpgrade("tptc_sb",21)?3.2:hasUpgrade("tptc_sb",14)?2:hasUpgrade("tptc_sb",11)?1.15:1)];
		},
		effectDescription() {
...
addLayer("tptr_sg", {
		...
		effectBase() {
			...
			if (hasUpgrade("tptr_g", 31) && player.tptr_i.buyables[12].gte(2)) base = base.times(upgradeEffect("tptr_g", 31));
			if (hasUpgrade("tptr_ba", 32)) base = base.times(upgradeEffect("tptr_ba", 32));
			if (hasUpgrade("tptr_hn", 52)) base = base.times(buyableEffect("tptr_o", 12));
		...
		effect() {
			...
			return [eff,player.tptr_sg.points.add(1).pow(hasUpgrade("tptc_sg",21)?2:1).pow(hasUpgrade("tptc_sg",22)?2:1)];
		},
		effectDescription() {
	...
	getResetGain() {
		...
		challenges: {
			...
			31: {
				...
				rewardEffect() { 
					let eff = Decimal.mul(9, Decimal.add(player.timePlayed, 1).cbrt()).plus(1).pow(challengeCompletions("tptr_h", 31)).times(tmp.tptr_n.realDustEffs2?tmp.tptr_n.realDustEffs2.blueOrange:new Decimal(1));
			...
			32: {
				...
				rewardEffect() { 
					...
					eff =eff.times(tmp.tptr_n.realDustEffs2?tmp.tptr_n.realDustEffs2.blueOrange:new Decimal(1));
					if (!eff.eq(eff)) eff = new Decimal(1);
					return eff;
				},
			...
			42: {
				name: "Unquirky",
				completionLimit: 1,
				challengeDescription: "Force a Row 7 reset in TPTC. You can't gain Quirk Energy in TPTC & TPTR.",
				unlocked() { return player.tm.buyables[7].gte(35) },
				goal() { return new Decimal("e2592e11") },
				currencyDisplayName: "points",
				currencyInternalName: "points",
				rewardDescription: "Quirk Layer cost base is reduce in TPTC & TPTR. Reduce Nebula requirement in TPTR.",
			},
...
addLayer("tptr_q", {
	...
	getResetGain() {
        ...
        branches: ["tptr_e"],
		enGainMult() {
			if(inChallenge("tptr_h", 42))return new Decimal(0);
			let mult = new Decimal(1);
		...
		update(diff) {
			...
			if(inChallenge("tptr_h", 42))player.tptr_q.energy = new Decimal(0);
	...
	effectDescription() { // Optional text to describe the effects
		...
		freeLayers() {
			...
			if (player.tptr_i.buyables[11].gte(3)) l = l.plus(buyableEffect("tptr_s", 18));
			return l;
		},
...
addLayer("tptr_o", {
	name: "tptr_o", // This is optional, only used in a few places, If absent it just uses the layer id.
		...
		exponent() { 
			...
			if (hasUpgrade("tptr_hn", 25)) exp = exp.times(upgradeEffect("tptr_hn", 25));
			if (player.tm.buyables[7].gte(41)) exp = exp.times(buyableEffect("tptr_o", 32));
			return exp;
		...
		effect3() { if(!player.tptr_o.unlocked)return new Decimal(1);
			if(player.tptr_o.points.gte("1e900")){
				return player.tptr_o.points.add(1).log10().add(1).log10().add(1);
			}
		...
		buyables: {
			...
			22: {
				...
				gain() { return player.tptr_o.buyables[11].div(1e150).pow(3).pow(tmp.tptr_o.buyableGainExp).floor() },
				effect() { 
					let eff = player[this.layer].buyables[this.id].times(tmp.tptr_o.multiplyBuyables).plus(1).pow(tmp.tptr_o.solPow).log10().root(10).plus(1);
					return eff;
				},
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    let display = ("Gaining "+formatWhole(tmp[this.layer].buyables[this.id].gain)+" Noval Remnants per second, based on Solar Cores.\n"+
					"Amount: " + formatWhole(player[this.layer].buyables[this.id])+((tmp.tptr_o.multiplyBuyables||new Decimal(1)).eq(1)?"":(" x "+format(tmp.tptr_o.multiplyBuyables))))+"\n"+
					(("Effect: Multiply Honour gain (unaffected by softcap) and Purple, Blue, & Orange Dust gain by "+format(data.effect)+"x"))
					return display;
                },
                unlocked() { return player.tm.buyables[7].gte(38) }, 
			...
			32: {
				...
				gain() { return player.tptr_o.energy.div("1e200000").root(100).pow(tmp.tptr_o.buyableGainExp).floor() },
				effect() {
					return player[this.layer].buyables[this.id].times(tmp.tptr_o.multiplyBuyables).plus(1).pow(tmp.tptr_o.solPow).log10().plus(1).log10().plus(1).log10().div(1.6).plus(1)
				},
				display() {
					let data = tmp[this.layer].buyables[this.id]
					return ("Gaining "+formatWhole(data.gain)+" Combustion Gas per second, based on Solar Energy.\n"+
					"Amount: "+formatWhole(player[this.layer].buyables[this.id])+((tmp.tptr_o.multiplyBuyables||new Decimal(1)).eq(1)?"":(" x "+format(tmp.tptr_o.multiplyBuyables)))+"\n"+
					(("Effect: Multiply the Solarity gain exponent by "+format(data.effect)+".")))
				},
                unlocked() { return player.tm.buyables[7].gte(41) }, 
                canAfford() { return true; },
                buy() { 
                },
                style: {'height':'140px', 'width':'140px', 'font-size':'9px'},
...
addLayer("tptr_ss", {
		...
		effBase() {
			...
			if (hasUpgrade("tptr_hn", 35)) base = base.times(upgradeEffect("tptr_hn", 35));


			if (hasUpgrade("tptr_t", 41) && player.tptr_i.buyables[12].gte(4)) base = base.pow(1.5);
			return base;
		},
		effect() { 
		...
		milestones: {
			...
			3: {
				...
				effectDescription: "Autobuy first 5 Space Buildings, Space Buildings won't cost any space.",
			},
		},
...
gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1);
		if (hasUpgrade("tptr_hn", 15)) mult = mult.times(challengeEffect("tptr_h", 32));
            return mult.times(tmp.tptr_n.realDustEffs2?tmp.tptr_n.realDustEffs2.purpleBlue:new Decimal(1));
        },
        gainExp() { // Calculate the exponent on main currency from bonuses
            return new Decimal(1)
        },
		...
		upgrades: {
			...
			32: {
				...
				effect() { 
					...
					if (hasUpgrade("tptr_hn", 44)) eff = eff.times(upgradeEffect("tptr_p", 44));
...
addLayer("tptr_ps", {
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
			if (player.tptr_i.buyables[11].gte(2)) mult = mult.div(buyableEffect("tptr_s", 17));
            return mult
        },
        gainExp() { // Calculate the exponent on main currency from bonuses
            return new Decimal(1)
	...
	getResetGain() {
		let ret=getResetGain(this.layer,"static").add(player[this.layer].points);
		let sc1=layers[this.layer].getSc1();
		if(ret.gte(30))ret=ret.div(sc1).sqrt().mul(sc1).floor().max(30);
		return ret.sub(player[this.layer].points).max(1);
	},
	getNextAt(canMax) {
		if (!tmp[this.layer].canBuyMax) canMax = false
		let amt = player[this.layer].points.plus((canMax&&tmp[this.layer].baseAmount.gte(tmp[this.layer].nextAt))?tmp[this.layer].resetGain:0);
		let sc1=layers[this.layer].getSc1();
		if(amt.gte(30))amt=amt.div(sc1).pow(2).mul(sc1).max(30).ceil();
		let extraCost = Decimal.pow(tmp[this.layer].base, amt.pow(tmp[this.layer].exponent).div(tmp[this.layer].gainExp)).times(tmp[this.layer].gainMult)
		let cost = extraCost.times(tmp[this.layer].requires).max(tmp[this.layer].requires)
		return cost;
	},
		...
		soulGainMult() {
			...
			if (player.tptr_i.buyables[11].gte(1)) mult = mult.times(buyableEffect("tptr_s", 16));
...
addLayer("tptr_hn", {
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
		if(player.tptr_i.best.gte(1))mult = mult.mul(player.tptc_sp.points.add(1e10).log10().log10().pow(10).pow(hasUpgrade("tptc_sp",43)?1.5:1));
	...
	if (player.tptr_ma.unlocked) mult = mult.times(tmp.tptr_ma.effect);
		...
		upgrades: {
			...
			15: {
				...
				description: "Option D reward boost magic and balance energy.",
				cost() { return new Decimal(1e45) },
				unlocked() { return player.tm.buyables[7].gte(38) },
			},
			21: {
			...
			25: {
				...
				cost() { return new Decimal(3e47) },
				unlocked() { return player.tm.buyables[7].gte(38) },
				effect() { return player.tptr_hs.points.times(player.tptr_n.points.pow(3)).plus(1).log10().plus(1).log10().plus(1) },
				effectDisplay() { return format(tmp.tptr_hn.upgrades[25].effect)+"x" },
				style: {"font-size": "9px"},
			},
			31: {
			...
			35: {
				...
				cost() { return new Decimal(5e52) },
				unlocked() { return player.tm.buyables[7].gte(38) },
				effect() { return player.tptr_n.purpleDust.times(player.tptr_n.blueDust).plus(1).pow(10) },
				effectDisplay() { return format(tmp.tptr_hn.upgrades[35].effect)+"x" },
				formula: "(B*P+1)^10",
			},
			41: {
			...
			45: {
				...
				effectDisplay() { return format(tmp.tptr_hn.upgrades[45].effect)+"x" },
			},
			51: {
				...
				unlocked() { return player.tm.buyables[7].gte(36) },
				effect() { return player.tptr_hn.total.plus(1).pow(5) },
				effectDisplay() { return "/"+format(tmp.tptr_hn.upgrades[51].effect) },
				formula: "(x+1)^5",
				style: {"font-size": "8px"},
			}, 
			52: {
...
addLayer("tptr_hs", {
        ...
        color: "#dfdfff",
        requires() { 
		if(player.tm.buyables[7].gte(41))return new Decimal(360);
		...
		if(player.tm.buyables[7].gte(35))return new Decimal(840);
        ...
        exponent() { 
			...
			if (player.tptr_i.buyables[11].gte(4)) exp = exp.times(buyableEffect("tptr_s", 19));
			return exp;
		}, // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1);
		if(player.tptr_i.best.gte(1))mult = mult.mul(player.tptc_hs.points.add(1e10).log10().log10().pow(hasUpgrade("tptc_sp",43)?1.5:1));
			if (hasUpgrade("tptr_g", 35) && player.tptr_i.buyables[12].gte(2)) mult = mult.times(upgradeEffect("tptr_g", 35));
		...
		buyables: {
			...
			11: {
				...
				scaleRate() {
					...
					if (hasUpgrade("tptr_t", 32) && player.tptr_i.buyables[12].gte(4)) rate = new Decimal(2/3);
                ...
                buyMax() {
					let y = player.tptr_hs.points.max(1).log10().root(.9);
			...
			26: {
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.tptr_hs.buildLim)+"\n\n\
					Senary Space Building Effect: ^"+format(tmp[this.layer].buyables[this.id].effect))
					return display;
                },
                unlocked() { return player[this.layer].unlocked && player.tptr_i.buyables[11].gte(1) }, 
                canAfford() {
				...
				effect() {
					return softcap(player[this.layer].buyables[this.id],new Decimal(3),new Decimal(0.2)).times(tmp.tptr_hs.buildingPower).pow(1.1).div(1.2).plus(1);
				},
                buy() { 
					player.tptr_hs.spentHS = player.tptr_hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.tptr_hs.buildLim);
			...
			27: {
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.tptr_hs.buildLim)+"\n\n\
					Septenary Space Building Effect: ^"+format(tmp[this.layer].buyables[this.id].effect))
					return display;
                },
                unlocked() { return player[this.layer].unlocked && player.tptr_i.buyables[11].gte(2) }, 
                canAfford() {
				...
				effect() {
					return softcap(player[this.layer].buyables[this.id],new Decimal(3),new Decimal(0.2)).times(tmp.tptr_hs.buildingPower).div(5).plus(1);
				},
                buy() { 
					player.tptr_hs.spentHS = player.tptr_hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.tptr_hs.buildLim);
			...
			28: {
				...
				effect() {
					return softcap(player[this.layer].buyables[this.id],new Decimal(3),new Decimal(0.2)).times(tmp.tptr_hs.buildingPower).div(1.15).plus(1);
				},
                buy() { 
					player.tptr_hs.spentHS = player.tptr_hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.tptr_hs.buildLim);
			...
			29: {
				...
				effect() {
					return softcap(player[this.layer].buyables[this.id],new Decimal(3),new Decimal(0.2)).times(tmp.tptr_hs.buildingPower).div(5).plus(1).min("1e4");
				},
                buy() { 
					player.tptr_hs.spentHS = player.tptr_hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.tptr_hs.buildLim);
			...
			30: {
				...
				effect() {
					return softcap(player[this.layer].buyables[this.id],new Decimal(3),new Decimal(0.2)).times(tmp.tptr_hs.buildingPower).sqrt().div(1.5).plus(1);
				},
                buy() { 
					player.tptr_hs.spentHS = player.tptr_hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.tptr_hs.buildLim);
...
addLayer("tptr_n", {
        ...
        requires() { return new Decimal(hasChallenge("tptr_h",42)?"1e280":"1e320") }, // Can be a function that takes requirement increases into account
		...
		tabFormat() { 
			...
			["column", 
				...
				(second?["column", [["display-text", ("Multiply Magic gain by <span style='color: #ee82ee; font-size: 24px'>"+format(tmp.tptr_n.dustEffs2.purpleBlue)+"</span>"+(tmp.nerdMode?" (Effect Formula: (purple*blue+1)^10)":" (based on Purple & Blue Dust)"))]], {"background-color": "rgba(238, 130, 238, 0.25)", width: "50vw", padding: "10px", margin: "0 auto"}]:[]),
				...
				(second?["column", [["display-text", ("Multiply the Time Capsule limit base by <span style='color: #94de95; font-size: 24px'>"+format(tmp.tptr_n.dustEffs2.orangePurple)+"</span><br>"+(tmp.nerdMode?" (Effect Formula: (orange*purple+1)^0.6)":" (based on Orange & Purple Dust)"))]], {"background-color": "rgba(148, 222, 149, 0.25)", width: "50vw", padding: "10px", margin: "0 auto"}]:[]),
			]],
		...
		dustGainMult() {
			...
			if (hasUpgrade("tptr_g", 33) && player.tptr_i.buyables[12].gte(2)) mult = mult.times(upgradeEffect("tptr_g", 33));
			/*if (player.ge.unlocked) mult = mult.times(tmp.ge.rotEff);
		...
		realDustEffs2() {
			let a = hasUpgrade("tptr_hn", 53);
			let data = tmp.tptr_n.dustEffs2;
			return {
				purpleBlue: a?data.purpleBlue:new Decimal(1),
				blueOrange: a?data.blueOrange:new Decimal(1),
				orangePurple: a?data.orangePurple:new Decimal(1),
			}
		},
		...
		milestones: {
			1: {
				requirementDescription: "TPTR Level 37",
				done() { return player.tm.buyables[7].gte(37) },
				effectDescription: "Gain 100% of nebula energy gain per second.",
			},
		},
	 passiveGeneration(){
		 if(player.tm.buyables[7].gte(37))return 1;
		 return 0;
	 },
	secondariesAvailable() { return hasUpgrade("tptr_hn", 53)?3:0 },
...
addLayer("tptr_i", {
		name: "tptr_i", // This is optional, only used in a few places, If absent it just uses the layer id.
		...
		base() { return new Decimal("1e250") },
	effectDescription(){
		if(player.tptr_i.points.gte(1))return "reducing Imperium Building cost scaling in TPTC";else return "";
	},
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
            return mult
		...
		update(diff) {
			if (!player.tptr_i.unlocked) return;
			player.tptr_i.nb = player.tptr_i.nb.max(tmp.tptr_i.nbAmt);
			player.tptr_i.hb = player.tptr_i.hb.max(tmp.tptr_i.hbAmt);
		},
		nbAmt() {
			let amt = player.tptr_n.points.div(1).plus(1).log10().root(1.25)
			return amt.floor();
		},
		nextNB() {
			let next = Decimal.pow(10, player.tptr_i.nb.plus(1).pow(1.25)).sub(1).times(1);
			return next;
		},
		hbAmt() {
			let amt = player.tptr_hs.points.div(1e6).plus(1).log10().root(1.35)
			return amt.floor();
		},
		nextHB() {
			let next = Decimal.pow(10, player.tptr_i.hb.plus(1).pow(1.35)).sub(1).times(1e6);
			return next;
		},
		tabFormat: ["main-display",
			"prestige-button",
			"resource-display",
			"milestones",
			["display-text", function() { return player.tptr_i.unlocked?("You have "+formatWhole(player.tptr_i.nb)+" Nebulaic Bricks (next at "+format(tmp.tptr_i.nextNB)+" Nebula Energy)"):"" }],
			["display-text", function() { return player.tptr_i.unlocked?("You have "+formatWhole(player.tptr_i.hb)+" Hyperspatial Bricks (next at "+format(tmp.tptr_i.nextHB)+" Hyperspace Energy)"):"" }],
			"blank",
			"buyables",
		],
		milestones: {
			0: {
				requirementDescription: "1 Total Imperium Brick",
				done() { return player.tptr_i.total.gte(1) || player.tptr_i.best.gte(1) },
				effectDescription: "Life in TPTC boost Nebula in TPTR, Super-Prestige in TPTC boost Honour in TPTR, Hyperspace Energy in TPTC boost Hyperspace Energy in TPTR.",
			},
		},
		...
		buyables: {
			...
			11: {
				...
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    let cost = { ib: x.times(1.4).pow(1.2).plus(1).pow(/*player.ma.current=="i"?player.i.buyables[12].div(4).plus(1):*/1).floor(), nb: x.pow(1.4).times(2).plus(3).pow(/*player.ma.current=="i"?player.i.buyables[12].div(6).plus(1):*/1).floor() }
					return cost;
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id];
                ...
                canAfford() {
					let cost = tmp[this.layer].buyables[this.id].cost
                    return player.tptr_i.unlocked && (cost.ib?player.tptr_i.points.gte(cost.ib):true) && (cost.nb?player.tptr_i.nb.gte(cost.nb):true) && (cost.hb?player.tptr_i.hb.gte(cost.hb):true) && player[this.layer].buyables[this.id].lt(tmp[this.layer].buyables[this.id].cap)},
                buy() { 
                    cost = tmp[this.layer].buyables[this.id].cost
					if (cost.ib) player.tptr_i.points = player.tptr_i.points.sub(cost.ib);
					if (cost.nb) player.tptr_i.nb = player.tptr_i.nb.sub(cost.nb);
					if (cost.hb) player.tptr_i.hb = player.tptr_i.hb.sub(cost.hb);
                    player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].add(1)
                },
                buyMax() {
					// later :)
			...
			12: {
				...
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    let cost = { ib: x.pow(1.2).plus(1).pow(/*player.ma.current=="i"?player.i.buyables[11].div(2).plus(1):*/1).floor(), hb: x.pow(1.6).plus(4).pow(/*player.ma.current=="i"?player.i.buyables[11].div(5).plus(1):*/1).floor() }
					return cost;
                },
				formulas: {
					ib: "x^1.2+1",
